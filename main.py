...
def start(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text(
        "🥶 В условиях нашего климата необходимо прогревать двигатель перед поездкой. Даже если на улице несильный мороз. Это занижает износ двигателя. В конце концов просто приятно съесть в прогретый автомобиль. Какую систему выберете?",
...
def autostart_choice(update: Update, context: CallbackContext) -> int:
    ...
    # Сообщение 2.2
    update.message.reply_text(
        "🙄 Существует устаревший способ управления системы через брелок. Проблема в том, что сигнал тревоги от автомобиля до брелка не всегда стабилен и есть шанс не получить сигнал тревоги. 📡 Через приложение в телефоне в независимости от вашего местоположения вы получите сообщение в случае тревоги. Что выберете?",
        reply_markup=ReplyKeyboardMarkup(
...
def control_choice(update: Update, context: CallbackContext) -> int:
    ...
    # Сообщение 3.2
    update.message.reply_text(
        "Если вы часто даете машину чужие руки и вам важно отслеживать точное местоположение автомобиля, то вам необходимо выбрать систему с GPS. Ваш вариант?",
        reply_markup=ReplyKeyboardMarkup(
